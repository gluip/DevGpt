...
namespace MyApp // Note: actual namespace depends on the project name.
{
    internal class Program
    {
        ...
        async static Task Main(string[] args)
        {
            ...
            var fullPrompt = promptGenerator.GetFullPrompt(new ICommand[]
            {
                new ReadFileCommand(),
            ...
            chatHandler.AddMessage(new ChatMessage(ChatRole.System, PromptGenerator.SystemPrompt));
            ...
            while (true)
            {
                ...
                chatHandler.AddMessage(new ChatMessage(ChatRole.Assistant, asistantReplyText));

                var asistantReply = JsonSerializer.Deserialize<Models.AssitantReply>(asistantReplyText);

                WriteReply(asistantReply);
                if (asistantReply.command != null)
                {
                    //prompt user in a y/n question
                    Console.WriteLine($"Do you want to execute {asistantReply.command} ? (y/n)");
                    var response = Console.ReadLine();
                    string result;
                    if (response == "y")
                    { 
                        result = CommandExecutor.Execute(asistantReply.command.name, asistantReply.command.args.ToArray());
                    }
                    else
                    {
                    ...
                    chatHandler.AddMessage(new ChatMessage(ChatRole.System, result));
                }
            }
        ...
        private static void WriteReply(Models.AssitantReply asistantReply)
        {
