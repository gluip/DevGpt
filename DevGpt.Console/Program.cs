...
namespace DevGpt.Console // Note: actual namespace depends on the project name.
{
    internal class Program
    {
        ...
        static async Task Main(string[] args)
        {
            ...
            chatHandler.AddMessage(new DevGptChatMessage(ChatRole.System, fullPrompt));
            ...
            while (true)
            {
                ...
                chatHandler.AddMessage(new DevGptChatMessage(ChatRole.Assistant, balancedText));
                ...
                try
                {
                    ...
                    if (asistantReply.command != null)
                    {
                        ...
                        {
                            chatHandler.AddMessage(new DevGptChatMessage(ChatRole.User, $"Command '{asistantReply.command.name}' not found. Proceed to formulate a concrete command."));
                            continue;
                        }
                        ...
                        System.Console.WriteLine($"Do you want to execute {asistantReply.command.name} ? (y/n)");
                        var response = System.Console.ReadLine();
                        string result;
                        string context = null;
                        if (response.ToLower() == "y")
                        {
                            var complexResult = await commandExecutor.Execute(asistantReply.command.name,
                                asistantReply.command.args.ToArray());
                            result = complexResult.Result;
                            context = complexResult.Context;
                        }
                        else if (response == "r")
                        {
                        ...
                        chatHandler.AddMessage(new DevGptChatMessage(ChatRole.User, result));
                        if (!string.IsNullOrEmpty(context))
                        {
                            chatHandler.AddMessage(new DevGptChatMessage(ChatRole.User, "Context : " + context){IsContext = true});
                            context = null;
                        }
                    }
                }
                ...
                {
                    chatHandler.AddMessage(new DevGptChatMessage(ChatRole.User, $"Unable to parse the response as json. Please provide a proper formatted respose.Error : "+e.Message));
